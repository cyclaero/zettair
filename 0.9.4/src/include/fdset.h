...
/* remove a specific file from the fdset. Ensure that you have unpinned
 * the fd before attempting this. Returns FDSET_OK on success and -errno on
 * failure.
 */
...
int fdset_debug_pin(struct fdset *set, unsigned int type, unsigned int fileno,
  unsigned long int offset, int whence, const char * src_file, int src_line);
