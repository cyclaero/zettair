 ...
 * by scripts/metric.py on Mon, 02 Mar 2009 06:38:45 GMT.  
 *
...
static enum search_ret parse(void *ptr, const char *str) {
    /* metric has no parameters, just ensure that parameters string is empty */
    if (str == NULL || *str_ltrim(str) == '\0') {
        return SEARCH_OK;
    } else {
...
/* processed contents from src/cosine.metric and src/metric.c follows... */
...
static enum search_ret post(struct index *idx, struct query *query, 
  struct search_acc_cons *acc, int opts, struct index_search_opt *opt) {
    /* METRIC_POST */
    const double Q_weight = search_qweight(query);
    ...
    while (1) {
        while (NEXT_DOC(&v, docno, f_dt)) {
            ...
            if (acc && (docno == acc->acc.docno)) {
                /* METRIC_PER_DOC */
                (acc->acc.weight) += (1 + log((query->term[qterm].f_qt))) * (1 + log(f_dt));
...
const struct search_metric * /* METRIC_NAME */ cosine 
  (struct search_metric *sm, int offsets) {
    sm->pre = pre;
    sm->post = post;
    sm->name = /* METRIC_QUOTED_NAME */ "cosine";
