 ...
 * by scripts/metric.py on Mon, 02 Mar 2009 06:38:43 GMT.  
 *
...
static enum search_ret parse(void *ptr, const char *str) {
    ...
    struct dirichlet_param *param = ptr;
    unsigned int i,
                 parts,
                 read = 0;
    char *dup;
...
/* processed contents from src/dirichlet.metric and src/metric.c follows... */
...
static enum search_ret post(struct index *idx, struct query *query, 
  struct search_acc_cons *acc, int opts, struct index_search_opt *opt) {
    ...
    while (acc) {
        ...
        /* METRIC_POST_PER_DOC */
        (acc->acc.weight) += Q_terms * log((param->mu) / ((param->mu) + (DOCMAP_GET_WORDS(idx->map, acc->acc.docno))));
...
const struct search_metric * /* METRIC_NAME */ dirichlet 
  (struct search_metric *sm, int offsets) {
    sm->pre = pre;
    sm->post = post;
    sm->name = /* METRIC_QUOTED_NAME */ "dirichlet";
