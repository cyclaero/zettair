  ...
  int ignore_version) {
    ...
    if ((buf = malloc(bufsize))
      ...
      && (((filename[size] = '\0'), fp = fopen(filename, "rb"))
        ...
        READ_MEMBER(fp, idx->vector_types, uint32_t);
  ...
  unsigned int terms) {
    ...
    if ((buf = malloc(storagep_size()))
      ...
      && fwrite(INDEX_HEADER, str_len(INDEX_HEADER), 1, fp)) {
        ...
        WRITE_MEMBER(fp, idx->vector_types, uint32_t);
...
struct index *index_new(const char *name, const char *config,
  unsigned int memory, int opts, struct index_new_opt *opt) {
    ...
    enum docmap_ret dm_ret;
    FILE *fp;
    unsigned int qthreads = DEFAULT_QUERY_THREADS;
    ...
    if (opts & INDEX_NEW_NO_OFFSETS) {
        idx->vector_types = VOCAB_VTYPE_DOC;
    } else {
        idx->vector_types = VOCAB_VTYPE_DOCWP;
    }

    if (opts & INDEX_NEW_QTHREADS) {
        qthreads = opt->qthreads + !opt->qthreads;
    }
    ...
    if (opts & INDEX_NEW_STEM) {
        if ((opt->stemmer == INDEX_STEM_PORTERS)
          && (idx->stem
            ...
            idx->stem = NULL; /* for unknown algorithm */
    ...
    if (opts & INDEX_NEW_STOP) {
        ...
        /* initialise index with a build stoplist of '.', to exclude
         * end-of-sentence markers. */
        if (!(idx->istop = stop_new(index_stemmer(idx), idx->stem))
          || stop_add_term(idx->istop, ".") != STOP_OK) {
            ERROR("loading default build stop list");
            index_rm(idx);
            index_delete(idx);
            return NULL;
        }
    ...
    /* create a lock that allows the set number of query threads (even though
     * they won't be able to query anything until build time), and one index
     * update thread (for now, multi-threaded build would be nice), and
     * disallow reads and writes from running concurrently. */
    if ((idx->biglock = mrwlock_new(qthreads, 1, 1)) == NULL) {
        ERROR("creating lock for new index");
        index_delete(idx);
        return NULL;
    }
    ...
    if (!(idx->post = postings_new(idx->params.tblsize, index_stemmer(idx),
        idx->stem, idx->istop, idx->vector_types & VOCAB_VTYPE_DOCWP))) {
    ...
    if ((idx->merger
      = pyramid_new(idx->fd, idx->tmp_type, idx->index_type, idx->vocab_type,
          PYRAMID_WIDTH, idx->vector_types & VOCAB_VTYPE_DOCWP,
          &idx->storage, NULL, NULL))
        == NULL) {
...
struct index *index_load(const char *name, unsigned int memory, int opts,
  struct index_load_opt *opt) {
    ...
    unsigned int root_fileno,
                 ...
                 vfileno,
                 vsize,
                 qthreads = DEFAULT_QUERY_THREADS;
    ...
    if (opts & INDEX_LOAD_QTHREADS) {
        qthreads = opt->qthreads;
    }
    ...
    if (!(idx->post = postings_new(idx->params.tblsize, index_stemmer(idx),
        idx->stem, NULL, idx->vector_types & VOCAB_VTYPE_DOCWP))) {
...
int index_rm(struct index *idx) {
    ...
    if (mrwlock_wlock(idx->biglock) == MRWLOCK_OK) {
        ...
        for (i = 0; i < idx->repos; i++) {
            if ((fdset_name(idx->fd, idx->repos_type, i, buf, FILENAME_MAX,
                &len, &write) == FDSET_OK)
              && (len <= FILENAME_MAX) && (write)) {
        ...
        for (i = 0; fdset_unlink(idx->fd, idx->docmap_type, i) == FDSET_OK;
          i++) ;
        ...
        for (i = 0; i < idx->vectors || i == 0; i++) {
            if ((fdset_name(idx->fd, idx->index_type, i, buf, FILENAME_MAX,
                &len, &write) == FDSET_OK)
              && (len <= FILENAME_MAX) && (write)) {
...
int index_expensive_stats(const struct index *idx,
  struct index_expensive_stats *stats) {
    ...
    if ((mret = mrwlock_rlock(idx->biglock)) == MRWLOCK_OK
      && ((vocab_pret
        = zpthread_mutex_lock((zpthread_mutex_t *) &idx->vocab_mutex))
          == ZPTHREAD_OK)
      && ((docmap_pret
        = zpthread_mutex_lock((zpthread_mutex_t *) &idx->docmap_mutex))
          == ZPTHREAD_OK)) {
        ...
        stats->vocab_pages
          = iobtree_pages(idx->vocab, &stats->vocab_leaves, NULL);
        ...
        zpthread_mutex_unlock((zpthread_mutex_t *) &idx->docmap_mutex);
        zpthread_mutex_unlock((zpthread_mutex_t *) &idx->vocab_mutex);
        mrwlock_runlock(idx->biglock);
        return 1;
    } else {
        if (docmap_pret == ZPTHREAD_OK) {
            zpthread_mutex_unlock((zpthread_mutex_t *) &idx->docmap_mutex);
        }
        if (vocab_pret == ZPTHREAD_OK) {
            zpthread_mutex_unlock((zpthread_mutex_t *) &idx->vocab_mutex);
        }
        if (mret == MRWLOCK_OK) {
            mrwlock_runlock(idx->biglock);
...
int index_stats(const struct index *idx, struct index_stats *stats) {
    /* note that we could lock the docmap because of the docmap_entries call,
     * but it's almost certainly safe */
    if (mrwlock_rlock(idx->biglock) == MRWLOCK_OK) {
        ...
        stats->doc_order_vectors = (idx->vocab_type & VOCAB_VTYPE_DOCWP)
          | (idx->vocab_type & VOCAB_VTYPE_DOC);
        stats->doc_order_word_pos_vectors
          = (idx->vocab_type & VOCAB_VTYPE_DOCWP);
        stats->impact_vectors = idx->vocab_type & VOCAB_VTYPE_IMPACT;
        stats->sorted = 1;
        mrwlock_runlock(idx->biglock);
        return 1;
    } else {
      ...
      && ((docmap_pret
        ...
        if (docmap_get_location(idx->map, docno, &source, &offset, &bytes,
            ...
            if (flags & DOCMAP_COMPRESSED) {
                ...
                if ((fd
                    = fdset_pin(idx->fd, idx->repos_type, source, 0, SEEK_SET))
                  && (instream = stream_new())
                  && (gunzipfilter
                  ...
                  && (buf = malloc(BUFSIZ))) {
                    ...
                    if (curroffset + instream->avail_out
                      >= offset + dst_offset + dstsize) {
                        ...
                        unsigned int lcloffset
                          = offset + dst_offset - curroffset;
...
void index_delete(struct index *idx) {
    if ((!idx->biglock || mrwlock_wlock(idx->biglock) == MRWLOCK_OK)
      && zpthread_mutex_lock(&idx->vocab_mutex) == ZPTHREAD_OK
      ...
      && zpthread_mutex_lock(&idx->perquery_mutex) == ZPTHREAD_OK) {
        ...
        while (idx->perquery) {
            struct index_perquery *next = idx->perquery->next;
            index_perquery_delete(idx->perquery);
            idx->perquery = next;
        }

        zpthread_mutex_unlock(&idx->perquery_mutex);
        zpthread_mutex_destroy(&idx->perquery_mutex);
        zpthread_mutex_unlock(&idx->docmap_mutex);
        zpthread_mutex_destroy(&idx->docmap_mutex);
        zpthread_mutex_unlock(&idx->vocab_mutex);
        zpthread_mutex_destroy(&idx->vocab_mutex);

        if (idx->biglock) {
            mrwlock_wunlock(idx->biglock);
            mrwlock_delete(idx->biglock);
            idx->biglock = NULL;
        }
    }
  ...
  unsigned int *aux_len) {
    ...
    if ((mret = mrwlock_rlock(idx->biglock)) == MRWLOCK_OK
      && zpthread_mutex_lock((zpthread_mutex_t *) &idx->docmap_mutex)
        ...
        dm_ret
          = docmap_get_trecno(idx->map, docno, aux_buf, aux_buf_len, aux_len);
        zpthread_mutex_unlock((zpthread_mutex_t *) &idx->docmap_mutex);
        mrwlock_runlock(idx->biglock);
...
#define FAIL()                                                                \
    ...
    if ((parsebuf = malloc(idx->params.parsebuf))
      ...
      && (filter = (struct stream_filter *) detectfilter_new(BUFSIZ, 0))) {
        ...
        /* we got an empty doc, ignore */
        if (bytes_read == 0) {
        	/* this doc is a waste of our time, get rid of it from fd set */
...
/* note that index_construct is not callable by the outside world, and so,
 * doesn't require locking stuff */
int index_construct(struct index *idx, int opts,
        ...
        for (i = 0; i < idx->vectors; i++) {
            ...
            if ((fdset_name(idx->fd, idx->tmp_type, i, dstfile, FILENAME_MAX,
              ...
              && (rename(srcfile, dstfile) == 0)) {
                ...
                assert(!CRASH);
                mrwlock_wunlock(idx->biglock);
                ERROR("swapping TMP and IDX type files around");
                return 0;
            }
        }
        ...
        for (i = 0; i < idx->vocabs; i++) {
            ...
            if ((fdset_name(idx->fd, idx->vtmp_type, i, dstfile, FILENAME_MAX,
              ...
              && (rename(srcfile, dstfile) == 0)) {
                ...
                assert(!CRASH);
                mrwlock_wunlock(idx->biglock);
                ERROR("swapping VTMP and VOCAB type files around");
                return 0;
            }
        }
...
struct index_perquery *index_perquery_fetch(struct index *idx) {
    if (zpthread_mutex_lock(&idx->perquery_mutex) == ZPTHREAD_OK) {
        struct index_perquery *curr = NULL;

        if ((curr = idx->perquery)) {
            idx->perquery = curr->next;
        } else if ((curr = index_perquery_new(idx))) {
...
int index_perquery_store(struct index *idx, struct index_perquery *pq) {
    if (zpthread_mutex_lock(&idx->perquery_mutex) == ZPTHREAD_OK) {
        pq->next = idx->perquery;
        idx->perquery = pq;
        zpthread_mutex_unlock(&idx->perquery_mutex);
        return 1;
    } else {
...
struct index_perquery *index_perquery_new(struct index *idx) {
    struct index_perquery *pq = NULL;

    if ((pq = malloc(sizeof(*pq)))
      && (pq->sum = summarise_new(idx))) {
        pq->next = NULL;
        return pq;
    } else {
        if (pq) {
...
void index_perquery_delete(struct index_perquery *pq) {
    summarise_delete(pq->sum);
    pq->sum = NULL;
    free(pq);
}
