...
#define INDEX_SUMMARYLEN 256        /* Maximum size of document summary */
#define INDEX_TITLELEN 256          /* Maximum size of document title */
#define INDEX_AUXILIARYLEN 256      /* Maximum size of auxiliary fields */
...
enum index_new_opts {
    ...
    INDEX_NEW_STEM = (1 << 4),          /* use stemming algorithm (passed as stemmer) to stem the index */
    INDEX_NEW_MAXFILESIZE = (1 << 5),   /* set maximum filesize created */
    INDEX_NEW_ENDIAN = (1 << 6),        /* set the endian-ness of the generated index (supply bigendian, which is 0 for little-endian and non-zero for bigendian) */
    INDEX_NEW_STOP = (1 << 8),          /* stop at indexing time, using the file given as stop_file */
    INDEX_NEW_QSTOP = (1 << 11),        /* stop at query time, using the file given as qstop_file (or NULL for default stoplist) */
    INDEX_NEW_TABLESIZE = (1 << 9),     /* dictate how large the postings hashtable is */
    INDEX_NEW_PARSEBUF = (1 << 10),     /* dictate how large the parsing buffer is */
    INDEX_NEW_NO_OFFSETS = (1 << 12),   /* build the index without offsets */
    INDEX_NEW_QTHREADS = (1 << 13)      /* number of simultaneous query threads to allow is supplied */
};
...
/* create a new, empty index.  config can now be NULL if default compile-time configuration is to be used */
...
enum index_load_opts {
    ...
    INDEX_LOAD_IGNORE_VERSION = (1 << 5),   /* if the index file format version doesn't match, warn, but try loading the index anyway. */
    INDEX_LOAD_TABLESIZE = (1 << 7),        /* dictate how large the postings hashtable is */
    INDEX_LOAD_PARSEBUF = (1 << 8),         /* dictate how large the parsing buffer is */
    INDEX_LOAD_QSTOP = (1 << 11),           /* stop at query time, using the file given as qstop_list (or NULL for default stoplist) */
    INDEX_LOAD_DOCMAP_CACHE = (1 << 12),    /* specify which values to cache in-memory when the docmap loads */
    INDEX_LOAD_QTHREADS = (1 << 13)         /* number of simultaneous query threads to allow is supplied */
};
...
/* read an index off of disk. memory is an indication of how much wants the system to use; INDEX_MEMORY_UNLIMITED leaves it up to the system. */
struct index *index_load(const char *name, unsigned int memory, int opts, struct index_load_opt *opt);

/* remove the index from disk.  Returns 0 on success, negative failure.  Does _not_ perform index_delete.*/
...
struct index_stats {
    ...
    unsigned int updates;                       /* number of times the index has actually been updated (not necessarily number of times index_add was called) */
    ...
    unsigned int doc_order_word_pos_vectors;    /* whether has doc-order with word positions vectors are in index */
    unsigned int impact_vectors;                /* indicates if impact ordered vectors are in index */
};

/* struct to record statistics about the index that take a while to calculate */
struct index_expensive_stats {
    ...
    unsigned int vocab_pages;                   /* total number of pages (incl leaves) in vocab btree */
    ...
    double allocated_files;                     /* total size of space allocated to lists in files */
    double vocab_info;                          /* size of non-structural information stored in the vocabulary */
    double vocab_structure;                     /* size of structural information stored in the vocabulary */
    /* XXX: size of query processing vectors? */
    /* XXX: docmap aux string size? */
};

/* get statistics about the index, (statistics are written into stats structure) returning true on success. */
int index_stats(const struct index *idx, struct index_stats *stats);

/* index_expensive_stats returns stats that take time to calculate. */
int index_expensive_stats(const struct index *idx, struct index_expensive_stats *stats);
...
struct index_result {
    ...
    char summary[INDEX_SUMMARYLEN + 1];         /* a summary of the document */
    char title[INDEX_TITLELEN + 1];             /* the title of the document */
    char auxilliary[INDEX_AUXILIARYLEN + 1];    /* auxilliary information about the document, such its TREC number or URL */
};

/* options that can be passed to index_search. Note that OKAPI and COSINE are mutually exclusive. */
enum index_search_opts {
    INDEX_SEARCH_NOOPT = 0,                     /* pass this to indicate no flags */
    INDEX_SEARCH_OKAPI_RANK = (1 << 0),         /* use Okapi ranking scheme with supplied parameters (must supply k1, k3, b as doubles) */
    INDEX_SEARCH_PCOSINE_RANK = (1 << 1),       /* use pivoted cosine ranking scheme with supplied pivot point (must supply pivot as a double). 
                                                   Note that a pivot of 0 effectively turns off pivoting, and 0.2 is a reasonable starting value */
    INDEX_SEARCH_COSINE_RANK = (1 << 7),        /* use basic cosine measurement with query length normalisation */
    INDEX_SEARCH_WORD_LIMIT = (1 << 2),         /* supply (as unsigned int) the maximum number of words used from the query. A default limit applies if not supplied */

    /* use language model ranking with dirichlet smoothing as found in 
       Zhai & Lafferty, 'A study of smoothing methods', ACM TOIS 2004 vol 22 num 2, 179-214.
       Must provide mu parameter as an option */
    ...
    /* provide a text string describing the ranking to be used,
       as well as a text string describing the parameters to be used. */
    INDEX_SEARCH_DYNAMIC_RANK = (1 << 11),

    /* query biased document summary type */
    INDEX_SEARCH_SUMMARY_TYPE = (1 << 6),

    /* explicitly set the accumulator limit, rather than using the default ACCUMULATOR_LIMIT adjusted to a minimum of 1%.
       Note that if you explicitly set the accumulator limit, then _no_ adjustment is made for either collection size or number of results requested. */
...
unsigned int index_retrieve_doc_bytes(const struct index *idx, unsigned long int docno);
 
/* retrieve document statistics from the index. the length (written into bytes),
 * number of words in (written into words), number of distinct words in (written
 * into distinct_words) cosine weight of (written into weight) and auxilliary data
 * (written into aux) of docno are retrieved.  Returns true on success. */
int index_retrieve_stats(const struct index *idx, unsigned long int docno, unsigned int *bytes,
                         unsigned int *words, unsigned int *distinct_words, double *weight, const char **aux);

/* options that can be passed to index_add */
enum index_commit_opts {
    INDEX_COMMIT_NOOPT = 0,                 /* pass this to indicate no flags */
    INDEX_COMMIT_DUMPBUF = (1 << 1),        /* buffer update writing (supply size of buffer as unsigned int) */
...
enum index_add_opts {
    ...
    INDEX_ADD_ACCBUF = (1 << 0),            /* buffer updates (supply size of buffer as unsigned int) */
    INDEX_ADD_ACCDOC = (1 << 1),            /* buffer this number of documents (supply number as unsigned int) */
...
struct index_add_opt {
    unsigned int accbuf;
    unsigned int accdoc;
    const char *detected_type;              /* contains type that file was indexed with after method call */
};
...
/* write all changes to the index to disk (returns true on success) (XXX: talk about options) */
