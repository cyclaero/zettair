 ...
 *   METRIC_QUOTED_NAME : "name of the metric"
 ...
 *   METRIC_STRUCT : name of the struct (i.e. struct dirichlet_param)
 *                   that holds parameters for this metric.
...
static enum search_ret pre(struct index *idx, struct query *query, 
  int opts, struct index_search_opt *opt) {
    /* METRIC_STRUCT */ *param = (void *) &opt->u;
