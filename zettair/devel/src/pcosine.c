 ...
 * by scripts/metric.py on Mon, 02 Mar 2009 06:38:44 GMT.  
 *
...
static enum search_ret parse(void *ptr, const char *str) {
    ...
    struct pcosine_param *param = ptr;
    unsigned int i,
                 parts,
                 read = 0;
    char *dup;
...
/* processed contents from src/pcosine.metric and src/metric.c follows... */
...
static enum search_ret post(struct index *idx, struct query *query, 
  struct search_acc_cons *acc, int opts, struct index_search_opt *opt) {
    ...
    while (acc) {
        ...
        /* METRIC_POST_PER_DOC */
        (acc->acc.weight) /= (1.0 - (param->pivot)) + (param->pivot) * ((DOCMAP_GET_WEIGHT(idx->map, acc->acc.docno)) / avg_D_weight);
    ...
    while (1) {
        while (NEXT_DOC(&v, docno, f_dt)) {
            ...
            if (acc && (docno == acc->acc.docno)) {
                /* METRIC_PER_DOC */
                (acc->acc.weight) += w_qt * (1 + log(f_dt));
...
const struct search_metric * /* METRIC_NAME */ pcosine 
  (struct search_metric *sm, int offsets) {
    sm->pre = pre;
    sm->post = post;
    sm->name = /* METRIC_QUOTED_NAME */ "pcosine";
